...
let 
	...
	{ localParams are number of samples. }
	localParamArray =  [ 16000,8000,4000,2000,1000,500,250 ],	

	{ globalParams are of the form :  [([arity], arcProb, leafProb] }
	globalParamArray = [
				 ([2,2,2,2,2,2],		0.25,	0.75),
				 ([2,2,2,2,2,2,2],		0.25,	0.75),
				 ([2,2,2,2,2,2,2,2],		0.25,	0.75),
				 ([2,2,2,2,2,2,2,2,2],		0.25,	0.75),
				 ([2,2,2,2,2,2,2,2,2,2],	0.25,	0.75)

				{,
				 ([2,2,2,2,2,2],		0.75,	0.75),
				 ([2,2,2,2,2,2,2],		0.75,	0.75),
				 ([2,2,2,2,2,2,2,2],		0.75,	0.75),
				 ([2,2,2,2,2,2,2,2,2],		0.75,	0.75),
				 ([2,2,2,2,2,2,2,2,2,2],	0.75,	0.75),

				 ([2,6,20,3,3,8,2,4],		0.25,	0.75),
				 ([2,6,20,3,3,8,2,4],		0.75,	0.75),

				 ([16,16,16,16,16,16,16,16],	0.25,	0.75)
				}
 		],

	mappingOver = "NumSamples",
	...
	makeTrainData = lambda globalParams . lambda localParams . lambda fold . let
		arity = cmpnt 0 globalParams,
		arcProb = cmpnt 1 globalParams,
		leafProb = cmpnt 2 globalParams,
		numSamples = localParams,
		my = makeNet arity arcProb leafProb,
		model = cmpnt 0 my,
		params = cmpnt 1 my,
		dataX = generate model (fold * 100) () params numSamples,
		dataZ = map (lambda x . ()) (iota numSamples)
	in ( dataX, dataZ ),
	...
	makeTestData = lambda globalParams . lambda localParams . lambda fold . let
		...
		model = cmpnt 0 my,
		params = cmpnt 1 my,
		dataX = generate model 1234 () params numTestSamples,
		dataZ = map (lambda x . ()) (iota numTestSamples)
	in ( dataX, dataZ ),
	...
	makeTrueModel = lambda globalParams . lambda localParams . lambda fold . let
		arity = cmpnt 0 globalParams,
		arcProb = cmpnt 1 globalParams,
		leafProb = cmpnt 2 globalParams,
		numSamples = localParams,
	...
	search = [
			...
			{
			makeBNetLearner [ ("mmlLearner",WallaceCPT.learnerStruct) ],
			makeBNetLearner [ ("mmlLearner",MMLCPT.learnerStruct) ],    			
...
runFullTest = lambda globalParam . let
	...
	logPTraining = lambda localParam . lambda s . lambda fold . let 
		...
		kl = ( trueLogProbTestData - logProbTestData ) / numTestSamples
	in kl,
