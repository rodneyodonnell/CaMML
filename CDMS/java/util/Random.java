 ...
 *
 * This version has been modified to include Randomised Shuffling.
 * See section 3.2.2 of the book.
 * 16th September 2002
 * LF.
 *
 * @author  Frank Yellin
...
/*  Start of modifications by LF on 16th Sept 2002.
    Randomizing by shuffling.  See section 3.2.2.
*/

    private int K = 32;
    private long V[] = new long[K];
    ...
    synchronized protected int next(int bits) 
    {
      ...
      {
        fill = false;
        V[0] = (seed * multiplier + addend) & mask;
        for (int i = 1; i < K; i++)
          V[i] = (V[i-1] * multiplier + addend) & mask;
        seed = (V[K-1] * multiplier + addend) & mask;
        Y = seed;
      }
      int j = (int) (Y / ((mask+1L) / (long)K)); 
      Y = V[j];
      seed = (seed * multiplier + addend) & mask;
      V[j] = seed;
      return (int)(Y >>> (48 - bits));
    }

    public String toString()
    {
      return "RNG: Linear Congruential Randomised by Shuffling.";
    }
